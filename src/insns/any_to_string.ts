import { Disassembler } from "../disassembler";
import { ExecutionContext, ExecutionResult } from "../execution_context";
import Instruction from "../instruction";
import { Class } from "../runtime";
import { Object } from "../runtime/object";
import { RubyString } from "../runtime/string";

export default class AnyToString extends Instruction {
    async call(context: ExecutionContext): Promise<ExecutionResult> {
        const value = context.pop()!;
        const original = context.pop()!;

        if (value.klass === await RubyString.klass()) {
            context.push(value);
        } else {
            const name = original.klass.get_data<Class>().full_name;
            context.push(await RubyString.new(`#<${name}:${Object.object_id_to_str(original.object_id)}>`));
        }

        return null;
    }

    pops(): number {
        return 2;
    }

    pushes(): number {
        return 1;
    }

    disasm(fmt: Disassembler): string {
        return fmt.instruction("anytostring");
    }
}
